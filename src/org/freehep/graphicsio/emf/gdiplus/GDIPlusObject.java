...
/**
 *  The Object metafile record contains information on an object 
 *  which must be stored for use by a future metafile record. 
 *  Examples include pens, brushes, fonts, images, StringFormat 
 *  objects, and so on. Naturally, the data format of each type 
 *  of object differs, and so the Flags value is used to determine 
 *  which type of object is being stored.
 *  <p>
 *  Object metafile records contain a numeric index (0 to 255). 
 *  The first Object record to appear in the file will have an 
 *  index of 0, the next record 1, and so on. Applications which 
 *  process metafiles should store these objects in an index-based 
 *  collection so that a later record can access them. For example, 
 *  a future DrawRects record might reference the Pen with index 2, 
 *  and the application should ensure that that object is still 
 *  accessible. Since the index is only one byte, only 256 objects 
 *  can be stored at any time. Thus, a simple stack will not suffice 
 *  and applications will have to be prepared for new Object records 
 *  overwriting old ones with the same index. 
 *  
...
public class GDIPlusObject extends EMFPlusTag {
    ...
    public GDIPlusObject(int index, Stroke stroke, Paint brush) {
        this();
        if (!(stroke instanceof BasicStroke)) throw new IllegalArgumentException(getClass()+": can only handle Stroke of class BasicStroke");
	...
	public GDIPlusObject(int index, Shape shape, boolean windingFill) {
        ...
        try {
            ...
            path = p.getPath();
            flags = index | PATH; 
            pathFillMode = windingFill ? FILL_MODE_WINDING : FILL_MODE_ALTERNATE;
        } catch (IOException e) {
            // ignored
        }
	}
	...
	public EMFPlusTag read(int tagID, int flags, EMFInputStream emf, int len)
		...
		switch(type) {
            ...
            case PEN:
                ...
                emf.readUINT();  // additional flags, NOTE no join, endcap, miterlimit etc.
                emf.readUINT();  // unknown
	...
	public void write(int tagID, int flags, EMFOutputStream emf) throws IOException {
		...
		switch(type) {
            ...
            case PEN:
                ...
                emf.writeUINT(0x0000);  // additional flags, NOTE no join, endcap, miterlimit etc.
                emf.writeUINT(0x0000);  // unknown
    ...
    public String toString() {
        ...
        switch(type) {
            ...
            case PATH:
                ...
                sb.append("\n  n: "+path.length);
                for (int i=0; i<path.length; i++) {
                    sb.append("\n  0x"+Integer.toHexString(path[i].getType())+" ("+path[i].getX()+", "+path[i].getY()+")");
                }
                break;
            default:
                sb.append("UNKNOWN");
    ...
    private Paint readBrush(EMFInputStream emf) throws IOException {
        ...
        switch(brushType) {
            ...
            case BRUSH_TYPE_LINEAR_GRADIENT:
                ...
                return null;
/*                
            emf.writeUINT(paint.isCyclic() ? WRAP_MODE_TYLE_FLIP_XY : WRAP_MODE_CLAMP); // NOTE: check
...
*/
    ...
    private void writeBrush(EMFOutputStream emf, Paint brush) throws IOException {
        ...
        if (brush instanceof Color) {
            ...
            emf.writeUINT(paint.isCyclic() ? WRAP_MODE_TYLE_FLIP_XY : WRAP_MODE_TYLE_FLIP_Y); // NOTE: check
            float x1 = (float)paint.getPoint1().getX();
            float y1 = (float)paint.getPoint1().getY();
            float x2 = (float)paint.getPoint2().getX();
            float y2 = (float)paint.getPoint2().getY();
